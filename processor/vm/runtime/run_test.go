...
func TestRunCode(t *testing.T) {
	...
	//
	toName := common.Name("unichain.asset")

	//fmt.Println("in TestRunCode3 ...")
	if err := account.CreateAccount(common.Name("unichain"), senderName, "", 0, 0, senderPubkey, ""); err != nil {
		fmt.Println("create sender account error\n", err)
		return
	}

	if err := account.CreateAccount(common.Name("unichain"), receiverName, "", 0, 0, receiverPubkey, ""); err != nil {
		fmt.Println("create receiver account error\n", err)
		return
	}

	if err := account.CreateAccount(common.Name("unichain"), toName, "", 0, 0, receiverPubkey, ""); err != nil {
