...
func TestClientSubscribeInvalidArg(t *testing.T) {
	...
	check := func(shouldPanic bool, arg interface{}) {
		...
		client.UniSubscribe(context.Background(), arg, "foo_bar")
	}
	check(true, nil)
	check(true, 1)
...
func TestClientSubscribe(t *testing.T) {
	server := newTestServer("uni", new(NotificationTestService))
	defer server.Stop()
	client := dialInProc(server)
	defer client.Close()
	...
	nc := make(chan int)
	count := 10
	sub, err := client.UniSubscribe(context.Background(), nc, "someSubscription", count, 0)
	if err != nil {
...
// In this test, the connection drops while UniSubscribe is
...
func TestClientSubscribeClose(t *testing.T) {
	...
	go func() {
		sub, err = client.UniSubscribe(context.Background(), nc, "hangSubscription", 999)
		errc <- err
	}()
	...
	case err := <-errc:
		if err == nil {
			t.Errorf("UniSubscribe returned nil error after Close")
		}
		if sub != nil {
			t.Error("UniSubscribe returned non-nil subscription after Close")
		}
	case <-time.After(1 * time.Second):
		t.Fatalf("UniSubscribe did not return within 1s after Close")
	}
}
...
func TestClientNotificationStorm(t *testing.T) {
	...
	doTest := func(count int, wantError bool) {
		...
		// very quickly.
		nc := make(chan int)
		sub, err := client.UniSubscribe(ctx, nc, "someSubscription", count, 0)
		if err != nil {
			t.Fatal("can't subscribe:", err)
		}
