...
import (
	...
	ldb "github.com/unichainplatform/unichain/utils/fdb/leveldb"
...
func init() {
	...
	importCommand.Flags().StringVarP(&uniCfgInstance.NodeCfg.DataDir, "datadir", "d", uniCfgInstance.NodeCfg.DataDir, "Data directory for the databases ")
	importCommand.Flags().StringVarP(&uniCfgInstance.GenesisFile, "genesis", "g", "", "genesis json file")
...
func importChain(args []string) error {
	...
	fp := args[0]
	if len(args) == 1 {
		if err := importBlockchain(unisrv.BlockChain(), fp); err != nil {
			return fmt.Errorf("Import error: %v", err)
		}
	} else {
		for i := 0; i < len(args); i++ {
			if err := importBlockchain(unisrv.BlockChain(), args[i]); err != nil {
	...
	db := unisrv.ChainDb().(*ldb.LDBDatabase)
