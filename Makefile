...
define build
	@go build -ldflags " \
	-X github.com/unichainplatform/unichain/cmd/utils.commit=$(shell cat commit_hash.txt) \
	-X github.com/unichainplatform/unichain/cmd/utils.date=$(shell date '+%Y-%m-%d-%H:%M:%S') \
	-X 'github.com/unichainplatform/unichain/cmd/utils.goversion=$(shell go version)'" \
...
# build all targets 
.PHONY: all
all:check  build_uni build_unifinder

# build uni
.PHONY: build_uni
build_uni: commit_hash check 
	@echo "Building uni."
	$(call build,uni)


# build unifinder
.PHONY: build_unifinder 
build_unifinder: commit_hash check 
	@echo "Building unifinder."
	$(call build,unifinder)
